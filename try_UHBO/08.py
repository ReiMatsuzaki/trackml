...
def run(filename):
    model = models.UnrollingHelicesRt2(
        dbscan_features=["sina1", "cosa1", "z1", "z2", "x_y", "x_rt", "y_rt"],
        dbscan_weight  =[1.0,     1.0,     0.75, 0.2,  0.005, 0.05,   0.05])
    model.niter = 150
    path_to_input = os.path.join(path_to_trackml, "train_1")
    for event_id, hits, truth in load_dataset(path_to_input, parts=["hits", "truth"],
                                              skip=0, nevents=1):

        def Fun4BO(w_a1, w_z1, w_z2, w_xy, w_xy_rt, c_rt2, c_a1, niter):
